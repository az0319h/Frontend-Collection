...
// 1. 변수 선언
const statsInfoTarget = document.querySelector('.stats_info');
const communityPc = document.querySelector('.community_pc')
...
function handleScroll() {
    ...
    // 타겟 요소의 위치 계산
    let targetRect = statsInfoTarget.getBoundingClientRect();
    ...
    targetRect = communityPc.getBoundingClientRect();

    if (targetRect.top <= screenCenter && targetRect.bottom >= screenCenter) {
        communityPc.querySelector('.community_image_sub1').classList.add('visible')
        setTimeout(function() {
            communityPc.querySelector('.community_image_sub2').classList.add('visible')
        }, 500); // 1000s = 1초
    }
}
